...
export const BatchProgressBar: React.FC<BatchProgressBarProps> = ({ 
  ...
  const batchPercentage = progress.totalBatches > 0 ? Math.round((progress.currentBatch / progress.totalBatches) * 100) : 0;
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around', textAlign: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
              ...
              <Text type="secondary" style={{ fontSize: '12px' }}>Successful</Text>
            </div>
          </div>
          ...
          <div style={{ 
            ...
            <div style={{ maxHeight: '80px', overflowY: 'auto' }}>
              {progress.recentlyProcessed.slice(-3).map((file, index) => (
                <div key={index} style={{ fontSize: '12px', color: '#52c41a', marginBottom: 4 }}>
                  ✅ {file}
                </div>
              ))}
            </div>
            ...
            <div style={{ maxHeight: '100px', overflowY: 'auto' }}>
              {progress.errors.slice(-2).map((error, index) => (
                <div key={index} style={{ fontSize: '11px', marginBottom: 8 }}>
                  <Text code style={{ color: '#ff4d4f' }}>{error.file}</Text>
                  <br />
        ...
        {progress.isComplete && (
          <div style={{ 
            ...
            <CheckCircleOutlined style={{ color: '#52c41a', fontSize: '32px', marginBottom: 8 }} />
            <br />
            <Title level={4} style={{ color: '#52c41a', margin: 0 }}>
              Analysis Complete!
            </Title>
            <Text type="secondary">
              Successfully processed {progress.successful} out of {progress.total} resumes
            </Text>
          </div>
        )}
      </Space>
