...
export const RealTimeProgressModal: React.FC<RealTimeProgressModalProps> = ({ 
  ...
  const getStatusIcon = () => {
    if (progress.isComplete) {
      return <CheckCircleOutlined style={{ color: '#52c41a', fontSize: '28px' }} />;
    }
    ...
    >
      <Space direction="vertical" style={{ width: '100%', textAlign: 'center' }} size="large">
        ...
        <Title level={3} style={{ margin: 0, color: '#1890ff' }}>
          {progress.isComplete ? '🎉 Analysis Complete!' : '🤖 AI Resume Analysis'}
        </Title>
          ...
          <Progress
            ...
            format={(percent) => (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '24px', fontWeight: 'bold', color: getProgressColor() }}>
                  {percent}%
                </div>
        ...
        {!progress.isComplete && (
          <div style={{ width: '100%', marginBottom: '16px' }}>
            ...
            <Progress
              percent={progress.totalBatches > 0 ? Math.round((progress.currentBatch / progress.totalBatches) * 100) : 0}
              size="small"
              strokeColor="#722ed1"
              trailColor="#f0f0f0"
              showInfo={false}
        ...
        <div style={{ 
          ...
          <Card size="small" style={{ textAlign: 'center', minWidth: '80px', borderColor: '#52c41a' }}>
            <div style={{ color: '#52c41a', fontSize: '20px', fontWeight: 'bold' }}>
              {progress.successful}
            </div>
            <Text type="secondary" style={{ fontSize: '11px' }}>✅ Success</Text>
          </Card>
          
          <Card size="small" style={{ textAlign: 'center', minWidth: '80px', borderColor: '#ff4d4f' }}>
          ...
          <Card size="small" style={{ textAlign: 'center', minWidth: '80px', borderColor: '#1890ff' }}>
            <div style={{ color: '#1890ff', fontSize: '20px', fontWeight: 'bold' }}>
              {progress.total - progress.processed}
            </div>
            <Text type="secondary" style={{ fontSize: '11px' }}>⏳ Remaining</Text>
          </Card>
        </div>
        ...
        {progress.isComplete && (
          ...
          >
            <Text style={{ color: '#52c41a', fontSize: '14px', fontWeight: 500 }}>
              🎯 Ready! You can now review the analyzed resumes in the table below.
            </Text>
          </Card>
        )}
      </Space>
