...
import { Card, Table, Tag, Space, Button, Modal, Input, message, Typography, Spin } from "antd";
import {
  EyeOutlined,
  CheckCircleOutlined,
  CloseCircleOutlined,
...
export default function ResumeList() {
  ...
  const jobRequirement: JobRequirement = {
    ...
    description: "We are looking for a passionate Junior Developer eager to learn and grow within our development team. You will support projects, gain hands-on experience, and contribute to building scalable applications.",
  ...
  const recommendationTag = (rec?: string) => {
    ...
    let color = "default";
    if (rec.toLowerCase().includes("strong") || rec.toLowerCase().includes("excellent")) color = "green";
    else if (rec.toLowerCase().includes("consider") || rec.toLowerCase().includes("good")) color = "blue";
    else if (rec.toLowerCase().includes("low") || rec.toLowerCase().includes("weak")) color = "red";
  ...
  const handleSubmitLinks = async () => {
    ...
    try {
      ...
      if (data.success && data.analyses) {
        const newResumes: ResumeRow[] = data.analyses.map((analysis: any, index: number) => ({
        ...
        message.success(`Successfully processed ${newResumes.length} CV(s) and sorted by match score!`);
        setUploadOpen(false);
        setLinksInput("");
      } else {
      ...
      message.error(error.message || "Failed to process CVs. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 8, padding: 12, backgroundColor: '#f5f5f5', borderRadius: 6 }}>
            ...
            <br />
            <Typography.Text type="secondary">{jobRequirement.company} • {jobRequirement.location}</Typography.Text>
          </div>
        </div>
        
        <Typography.Paragraph>
          Paste CV links (one per line or separated by commas). The AI will analyze each CV and provide match scores.
        </Typography.Paragraph>
      ...
      >
        {viewing && (
          <div>
            <div style={{ marginBottom: 16 }}>
              <Typography.Title level={4} style={{ margin: 0 }}>{viewing.candidate}</Typography.Title>
              {viewing.email && <Typography.Text type="secondary">{viewing.email}</Typography.Text>}
            </div>
            ...
            {viewing.experience_years && (
              <div style={{ marginBottom: 16 }}>
                ...
                <Typography.Text>{viewing.experience_years} years</Typography.Text>
              </div>
            )}
