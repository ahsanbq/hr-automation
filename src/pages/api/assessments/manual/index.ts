...
export default async function handler(
  ...
  try {
    switch (method) {
      case "GET":
        return await handleGetManualMeetings(req, res, user!);
      case "POST":
        return await handleCreateManualMeeting(req, res, user!);
      default:
...
async function handleGetManualMeetings(
  req: NextApiRequest,
  res: NextApiResponse,
  ...
  if (meetingType && include.manualMeeting) {
    include.manualMeeting.where = {
...
async function handleCreateManualMeeting(
  req: NextApiRequest,
  res: NextApiResponse,
  user: { id: number; type: string; companyId?: number }
) {
  const {
    ...
    sequenceOrder,
    notes,
    // Manual meeting specific fields
    meetingLink,
    agenda,
    meetingType,
    meetingSummary,
  ...
  if (!jobPostId || !resumeId) {
    return res.status(400).json({
      error: "Missing required fields: jobPostId, resumeId",
    });
  }

  // Validate meeting type if provided
  const validMeetingTypes = ["TECHNICAL", "BEHAVIORAL", "SITUATIONAL"];
  if (meetingType && !validMeetingTypes.includes(meetingType)) {
    return res.status(400).json({
      error:
        "Invalid meeting type. Must be TECHNICAL, BEHAVIORAL, or SITUATIONAL",
    });
  }
  ...
  // Check if Manual meeting already exists for this resume
  ...
  if (existingStage) {
    return res.status(409).json({
      error: "A manual meeting already exists for this candidate",
    });
  }

  // Create the assessment stage and manual meeting in a transaction
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Create the manual meeting
    const manualMeeting = await tx.manualMeeting.create({
      data: {
    ...
    return { assessmentStage, manualMeeting };
  });
  ...
  return res.status(201).json({
    ...
    manualMeeting: result.manualMeeting,
  });
}
