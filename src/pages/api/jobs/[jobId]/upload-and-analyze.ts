...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`🤖 Starting AI analysis for ${uploadResults.length} uploaded resumes`);
    ...
    for (const analysis of analysisResponse.analyses) {
      if (analysis.success) {
        try {
          ...
          const uploadInfo = uploadResults.find(r => r.s3Url === analysis.resume_path);
    ...
    const response = {
      success: true,
      message: `${savedResumes.length} resumes uploaded and analyzed successfully${
