...
export default function MeetingManagementPage() {
  ...
  const fetchResumeData = async () => {
    ...
    try {
      ...
      if (resumeResponse.ok) {
        ...
        // Job data is already included in resumeData.jobPost
        setJob(resumeData.jobPost);
        ...
        if (resumeData.meetings && resumeData.meetings.length > 0) {
          ...
          // Initialize rating
          const parsed = existingMeeting.meetingRating
            ? parseInt(existingMeeting.meetingRating, 10)
            : 0;
          setRating(Number.isNaN(parsed) ? 0 : parsed);
        }
      } else {
  ...
  const saveRating = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/meetings/${meeting.id}`, {
        ...
        body: JSON.stringify({
          meetingRating: Math.max(0, Math.min(10, rating)),
        }),
      });
      if (response.ok) {
  ...
  return (
    <AppLayout
      title={`Meeting - ${resume?.candidateName || "Unknown Candidate"}`}
      subtitle={`${job?.jobTitle || "Unknown Job"} at ${
    ...
    >
      <Row gutter={24}>
          ...
          <Card title="Candidate Summary">
            ...
            {resume.summary && (
              <div>
                ...
                <Paragraph style={{ marginTop: 8 }}>{resume.summary}</Paragraph>
              </div>
            )}
          </Card>
        </Col>
        ...
        <Col span={6}>
          ...
          <Card title="Meeting Rating" style={{ marginTop: 16 }}>
            <Space direction="vertical" style={{ width: "100%" }}>
              ...
              <div style={{ fontSize: 12, color: "#666" }}>
                {rating ? `${rating}/10` : "No rating yet"}
              </div>
              <Button
                type="primary"
                onClick={saveRating}
